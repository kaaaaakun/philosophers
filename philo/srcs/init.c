...
/*   Updated: 2023/10/25 17:42:57 by tokazaki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

void	make_philo_threads(pthread_t *philo_pthread_arry, t_philo_status \
			*philo_data_arry, t_philo_routine_data *routine_data);
			...
			t_philo_routine_data *routine_data, t_mutex *mutex_data)
{
	...
	while (i < routine_data->num_of_philo)
	{
		...
		pthread_mutex_init(&philo_data_arry[i].eat_count_mutex, NULL);
